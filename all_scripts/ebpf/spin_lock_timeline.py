...
int kprobe___raw_spin_lock_irq(struct pt_regs *ctx) {
    struct data_t data = {};
...
int kretprobe___raw_spin_lock_irq(struct pt_regs *ctx) {
    u32 pid = bpf_get_current_pid_tgid() >> 32;
    struct data_t *datap;
    u64 duration;
...
def print_event(cpu, data, size):
    ...
    if event.kernel_stack_id >= 0:
        ...
        print("spinlock() PID: %d Duration: %d-%d ns Comm: %s Name: %s Caller: %s" % (event.pid, event.timestamp, event.timecost, event.comm, event.parentName, parent_function))
        print(stack_symbols)
    else:
        print("spinlock() PID: %d Duration: %d-%d ns Comm: %s Name: %s" % (event.pid, event.timestamp, event.timecost, event.comm, event.parentName))
