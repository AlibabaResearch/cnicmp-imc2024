...
client() {
    ...
    curl -s http://127.0.0.1:5000/seq_lock?op=obtain #>> noprint
    local pod_config=$1
    local output=$2
    ...
    curl -s http://127.0.0.1:5000/seq_lock?op=release #>> noprint
...
kill -9 `ps -ef | grep  mon_cpu | head -n 1 |  awk '{print $2}'`
kill -9 `ps -ef | grep  mon_cpu | head -n 1 |  awk '{print $2}'`
rm -r /home/cni/cnicmp_logs/mem_cpu_usage.log 
clean
gen_config
#prewarm
run_multi
