...
client() {
    ...
    ip=$(crictl inspectp $pid | grep -e "10\.14" | awk -F '\"' '{print $4}' | head -n 1)
    #echo $ip
...
launch_net_container() {
    net_test_pid=$(crictl runp --runtime=$runtime /home/cni/rund-test/net_test.yaml)
    #echo testtt$net_test_pid
    net_test_cid=$(crictl create --no-pull $net_test_pid $DIR/testd.json /home/cni/rund-test/net_test.yaml)
