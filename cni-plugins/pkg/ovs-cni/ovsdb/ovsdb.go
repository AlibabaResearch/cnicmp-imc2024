...
// CreateTapPort Create a tap port in OVS
func (ovsd *OvsBridgeDriver) CreateTapPort(tapIfName string, ovsIfType string, ofPortType string, ofPortRequest uint, vlanTag uint, trunks []uint, ovnPortName string) error {
	intfUUID, intfOp, err := createInterfaceOperation(tapIfName, ofPortRequest, ovnPortName, ovsIfType)
	if err != nil {
		return err
	}

	portUUID, portOp, err := createTapPortOperation(tapIfName, vlanTag, trunks, ofPortType, intfUUID)
	if err != nil {
		return err
	}
...
func createTapPortOperation(intfName string, vlanTag uint, trunks []uint, portType string, intfUUID ovsdb.UUID) (ovsdb.UUID, *ovsdb.Operation, error) {
	portUUIDStr := intfName
	portUUID := ovsdb.UUID{GoUUID: portUUIDStr}
...
func CleanPorts(ovsDriver *OvsBridgeDriver) error {
