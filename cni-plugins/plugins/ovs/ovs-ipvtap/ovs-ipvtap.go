...
type PluginConf struct {
	...
	MTU        uint   `json:"mtu,omitempty"`   // OVS internal port and ipvtap MTU, default bridge mtu
	...
	Mode       string `json:"mode,omitempty"`  // ipvtap mode: l2<default>, l3, l3s
	UseRPC     bool   `json:"rpc,omitempty"`   // create ovs port using rpc, default false
	UseCmd     bool   `json:"cmd,omitempty"`   // create ipvlan using cmd, default false
	DoDebug    bool   `json:"debug,omitempty"` // debug logger enabler
}

func main() {
	skel.PluginMain(cmdAdd, cmdCheck, cmdDel, version.All, buildversion.BuildString("ovs-ipvtap"))
}

func cmdAdd(args *skel.CmdArgs) error {
	ifName, err := virtdev.GetRandomIfNameWithPrefix("oipvt")
	debug.Logf("\n****** Creating ovs-ipvtap CNI for %s(%s) in %s ******\n", args.IfName, ifName, args.Netns)
	...
	// Step 2 - create ipvtap in container nns with master of the ovs port
	var ipvtapInterface *current.Interface
	if conf.UseCmd {
		ipvtapInterface, err = virtdevcmd.CreateIpvtap(
			conf.PortName,
			ifName,
			containerNNS,
			conf.MTU,
		...
		ipvtapInterface, err = virtdev.CreateIpvtap(
			conf.PortName,
			ifName,
			containerNNS,
			conf.MTU,
	...
	debugLog(conf, ifName, "CreateIpvtap ok")

	// Step 3 - IPAM and return results
	result := &current.Result{
		CNIVersion: current.ImplementedSpecVersion,
		Interfaces: []*current.Interface{ipvtapInterface},
		DNS:        conf.DNS,
	}
...
func parseConfig(stdin []byte) (*PluginConf, error) {
	...
	// Config validation
	if conf.IPAM.Type == "" {
		return &conf, fmt.Errorf("ovs-ipvtap needs IPAM configuration! i.e., none empty conf.IPAM.Type!")
	}

	if conf.BridgeName == "" {
		return &conf, fmt.Errorf("ovs-ipvtap should be provided with the ovs bridge name!")
	}

	if conf.PortName == "" {
		return &conf, fmt.Errorf("ovs-ipvtap should be provided with the ovs internal port name!")
	}
...
func doIPAM(conf *PluginConf, stdin []byte, result *current.Result, ifName string, containerNNS ns.NetNS) error {
	...
	for _, ipc := range ipamResult.IPs {
		// All addresses apply to the container ipvtap interface
