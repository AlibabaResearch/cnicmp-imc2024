...
type PluginConf struct {
	...
	MTU        uint   `json:"mtu,omitempty"`   // OVS internal port and ipvlan MTU, default bridge mtu
	...
	Mode       string `json:"mode,omitempty"`  // ipvlan mode: l2<default>, l3, l3s
...
func main() {
	skel.PluginMain(cmdAdd, cmdCheck, cmdDel, version.All, buildversion.BuildString("ovs-ipvlan"))
}

func cmdAdd(args *skel.CmdArgs) error {
	ifName, err := virtdev.GetRandomIfNameWithPrefix("oipvl")
	debug.Logf("\n****** Creating ovs-ipvlan CNI for %s(%s) in %s ******\n", args.IfName, ifName, args.Netns)
	...
	// Step 2 - create ipvlan in container nns with master of the ovs port
	var ipvlanInterface *current.Interface
	if conf.UseCmd {
		ipvlanInterface, err = virtdevcmd.CreateIpvlan(
			conf.PortName,
			ifName,
			containerNNS,
			conf.MTU,
		...
		ipvlanInterface, err = virtdev.CreateIpvlan(
			conf.PortName,
			ifName,
			containerNNS,
			conf.MTU,
	...
	debugLog(conf, ifName, "CreateIpvlan ok")

	// Step 3 - IPAM and return results
	result := &current.Result{
		CNIVersion: current.ImplementedSpecVersion,
		Interfaces: []*current.Interface{ipvlanInterface},
		DNS:        conf.DNS,
	}
...
func parseConfig(stdin []byte) (*PluginConf, error) {
	...
	// Config validation
	if conf.IPAM.Type == "" {
		return &conf, fmt.Errorf("ovs-ipvlan needs IPAM configuration! i.e., none empty conf.IPAM.Type!")
	}

	if conf.BridgeName == "" {
		return &conf, fmt.Errorf("ovs-ipvlan should be provided with the ovs bridge name!")
	}

	if conf.PortName == "" {
		return &conf, fmt.Errorf("ovs-ipvlan should be provided with the ovs internal port name!")
	}
...
func doIPAM(conf *PluginConf, stdin []byte, result *current.Result, ifName string, containerNNS ns.NetNS) error {
	...
	for _, ipc := range ipamResult.IPs {
		// All addresses apply to the container ipvlan interface
