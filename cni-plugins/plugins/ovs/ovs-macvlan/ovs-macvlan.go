...
type PluginConf struct {
	...
	MTU        uint   `json:"mtu,omitempty"`   // OVS internal port and macvlan MTU, default bridge mtu
	...
	DevMac     string `json:"dmac,omitempty"`  // macvlan MAC address, default random
...
func main() {
	skel.PluginMain(cmdAdd, cmdCheck, cmdDel, version.All, buildversion.BuildString("ovs-macvlan"))
}

func cmdAdd(args *skel.CmdArgs) error {
	ifName, err := virtdev.GetRandomIfNameWithPrefix("omcvl")
	debug.Logf("\n****** Creating ovs-macvlan CNI for %s(%s) in %s ******\n", args.IfName, ifName, args.Netns)
	...
	// Step 2 - create macvlan in container nns with master of the ovs port
	var macvlanInterface *current.Interface
	if conf.UseCmd {
		macvlanInterface, err = virtdevcmd.CreateMacvlan(
			conf.PortName,
			ifName,
			containerNNS,
			conf.MTU,
		...
		macvlanInterface, err = virtdev.CreateMacvlan(
			conf.PortName,
			ifName,
			containerNNS,
			conf.MTU,
	...
	debugLog(conf, ifName, "CreateMacvlan ok")
...
func parseConfig(stdin []byte) (*PluginConf, error) {
	...
	// Config validation
	if conf.IPAM.Type == "" {
		return &conf, fmt.Errorf("ovs-macvlan needs IPAM configuration! i.e., none empty conf.IPAM.Type!")
	}

	if conf.BridgeName == "" {
		return &conf, fmt.Errorf("ovs-macvlan should be provided with the ovs bridge name!")
	}

	if conf.PortName == "" {
		return &conf, fmt.Errorf("ovs-macvlan should be provided with the ovs internal port name!")
	}
