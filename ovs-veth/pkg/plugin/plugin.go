...
func attachIfaceToBridge(bridgeName string, ovsDriver *ovsdb.OvsBridgeDriver, hostIfaceName string, contIfaceName string, ofportRequest uint, vlanTag uint, trunks []uint, portType string, intfType string, contNetnsPath string, ovnPortName string) error {
	...
	// cmd := "ovs-vsctl"
	// cmd_args := []string{"add-port", bridgeName, hostIfaceName, "tag=" + strconv.Itoa(int(vlanTag))}
...
func CmdAdd(args *skel.CmdArgs) error {
	...
	/**** cnicmp ****/
	cnilogger.RecordEnd(id, "CNI_prepare", startT)
	/**** cnicmp ****/
	...
	/**** cnicmp ****/
	cnilogger.RecordEnd(id, "CNI_setupVeth", startT)
	/**** cnicmp ****/
	...
	/**** cnicmp ****/
	/**** cnicmp ****/
	cnilogger.RecordEnd(id, "CNI_log_overhead", startT)
	/**** cnicmp ****/
	...
	// add qos rules - add simple rate limiting rules
	if netconf.QOS > 0 {
		//if false {
		...
		_, err := http.Get(url)
		if err != nil {
			return fmt.Errorf("qos http-obtain failed: %v", err)
		}

		cmd := "ovs-vsctl"
		//cmd_args := []string{"add-flow", bridgeName, "dl_type=0x0800,nw_dst=" + result.IPs[0].Address.IP.String() + ",actions=output:" + vhostName}
		cmd_args := []string{"set", "port", hostIface.Name, "qos=@newqos" + id, "--", "--id=@newqos" + id, "create", "qos", "type=egress-policer", "other-config:cir=46000000", "other-config:cbs=2048"}
		//cmd_args := []string{"/home/cni/cnicmp/scripts/add_qos.sh", hostIface.Name, id}
		if err = exec.Command(cmd, cmd_args...).Run(); err != nil {
			return fmt.Errorf("cmdAdd: failed in adding ovs qos rules1:"+"set port "+hostIface.Name+" qos=@newqos"+id+" -- --id=@newqos"+id+" create qos type=egress-policer other-config:cir=46000000 other-config:cbs=2048"+" - %v", err)
			return err
		}
		cmd_args = []string{"set", "interface", hostIface.Name, "ingress_policing_rate=368000", "ingress_policing_burst=1000"}
		if err = exec.Command(cmd, cmd_args...).Run(); err != nil {
		...
		//}
		//url :=fmt.Sprintf("http://127.0.0.1:5000/?op=qosAdd&br_name=%s&port_name=%s", netconf.ACL, netconf.BrName, hostIface.Name)
	...
	if netconf.ACL > 0 {
		//url :=fmt.Sprintf("http://127.0.0.1:5000/?op=flowAdd&num=%d&br_name=%s&port_name=%s", netconf.ACL, netconf.BrName, hostIface.Name)
		rand_ip := randomIP()
		url := "http://127.0.0.1:5000/get_lock?op=obtain&rand_ip=" + rand_ip
		_, err := http.Get(url)
		if err != nil {
		...
		for acl_id := 0; acl_id < netconf.ACL; acl_id++ {
			...
			//cmd_args := []string{"add-flow", netconf.BrName, "ip,nw_src=30.0.0." + strconv.Itoa(acl_id + 1) + "/32,nw_dst=" + rand_ip + "/32,actions=output:" + hostIface.Name}
	...
	/**** cnicmp ****/
	cnilogger.RecordEnd(id, "CNI_add_user_rules", startT2)

	url2 := "http://127.0.0.1:5000/barrier"
	_, err = http.Get(url2)
	if err != nil {
	...
	// [lyz]
	// url2 := "http://127.0.0.1:5000/get_lock?op=obtain"
	// _, err = http.Get(url2)
	// if err != nil {
	// 	return fmt.Errorf("qos http-obtain failed: %v", err)
	// }

	if err = attachIfaceToBridge(netconf.BrName, ovsDriver, hostIface.Name, contIface.Name, netconf.OfportRequest, vlanTagNum, trunks, portType, netconf.InterfaceType, args.Netns, ovnPort); err != nil {
		return err
	}
	// url2 = "http://127.0.0.1:5000/get_lock?op=release"
	// _, err = http.Get(url2)
	// if err != nil {
	// 	return fmt.Errorf("qos http-release failed: %v", err)
	// }
	/**** cnicmp ****/
	cnilogger.RecordEnd(id, "CNI_attachInterface", startT)
	/**** cnicmp ****/
	...
	if netconf.IPAM.Type != "" {
		...
		// [lyz]
		/**** cnicmp ****/
		startT_0 := cnilogger.RecordStart()
		/**** cnicmp ****/
		...
		/**** cnicmp ****/
		cnilogger.RecordEnd(id, "CNI_IPAM_waitlinkip", startT_0)
		/**** cnicmp ****/

		/**** cnicmp ****/
		startT_1 := cnilogger.RecordStart()
		/**** cnicmp ****/
		err = contNetns.Do(func(_ ns.NetNS) error {
			...
			/**** cnicmp ****/
			cnilogger.RecordEnd(id, "CNI_IPAM_assignIP-mac", startT_2)
			/**** cnicmp ****/
		...
		/**** cnicmp ****/
		cnilogger.RecordEnd(id, "CNI_IPAM_assignIP", startT_1)
		/**** cnicmp ****/
	...
	cnilogger.RecordEnd(id, "CNI_IPAM", startT)
