...
for cni_id, cni_data in enumerate(data):
    for eid in range(len(cni_data)):
        ...
        plt.text(bar_width * (-int(len(data)/2) - 0.6 + cni_id) + eid * bar_offset, cni_data[eid][0] + 10, str(round(cni_data[eid][0], 1)), fontsize=14)
...
plt.savefig("./figs_fixed/cpu_consumption_opt_peak.png")
plt.savefig("./figs_fixed/cpu_consumption_opt_peak.pdf")
plt.show()
