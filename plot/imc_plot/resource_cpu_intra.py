...
def get_cpu_utils(name):
    ...
    for i in range(log_num):
        log_file = "/home/cni/cnicmp_logs/dataplane_logs_intra/{}-res-{}.server".format(name, i+1)
        data = []
...
names  = ["veth-kernel", "vhost", "ipvlan-not-tap-kernel", "ipvlan-kernel",  "tc-real-kernel"] #"macvlan", "macvtap",
# processing raw data
data = []
for name in names:
...
plt.tight_layout()
plt.savefig("./figs/resource_cpu_intra_camera.pdf")
plt.show()